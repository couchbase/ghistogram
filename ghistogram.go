...
// [n*i, n*(i+1)] or [n^i, n^(i+1)] or [n, n + initial]
...
func (g *Generator) getBin() *HistogramBin {
	var start, end uint64
	if g._factor == 0.0 {
		start = g._start
		g._start += uint64(g._initial)
		end = g._start
	} else {
		if g._initial != -1 {
			start = uint64(g._initial)
			end = uint64(float64(g._start) * g._factor)
			g._initial = -1
		} else {
			start = uint64(float64(g._start) * g._factor)
			end = uint64(float64(start) * g._factor)
			g._start = start
		}
	}
...
// Populates the histogram bins using the generator
func (gh *Histogram) fill(g *Generator) {
	for i := 0; i < len(gh._bins); i++ {
		gh._bins[i].assign(g.getBin())
	}
